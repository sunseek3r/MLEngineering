...
def _build_experiment_config(
    ...
    return replace(base_config, data=data_config, artifacts=artifacts_config)
...
def main() -> None:
    ...
    if not config.experiments.enabled:
        LOGGER.info("Experiments are disabled in config.yaml.")
        return

    experiment_results: list[dict[str, Any]] = []

    for experiment_run in config.experiments.runs:
